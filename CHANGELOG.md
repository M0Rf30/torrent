...
- Stop a piece-verification-success storage write failure (`MarkComplete` erroring) from being silently believed anyway: the piece is now treated like a failed hash check (redownloaded and re-verified) instead of being marked complete in memory while storage never actually recorded it, which used to leave memory and storage permanently diverged; also make the equivalent unchecked `MarkNotComplete` error in `storage.Piece.ReadAt`'s premature-EOF path observable via an expvar counter instead of being silently discarded (fork-local fix)
- Send a best-effort `Stopped` announce when dropping a torrent whose last announce attempt had errored, instead of silently deleting the announce state and never notifying the tracker the torrent left the swarm - previously any torrent unlucky enough to have its last announce fail before being dropped would never send `Stopped`, leaving stale swarm membership until the tracker's own peer timeout (fork-local fix)
- Fix `Client.DialFirst` constructing its `dialPool` without initializing `resCh`, which deadlocked the call (and leaked a goroutine per dialer) on a nil channel send/receive instead of panicking or returning (fork-local fix)
